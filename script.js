...
function popcity() {
    ...
    const cityList = ['London', 'Paris', 'Berlin', 'Los Angeles', 'Rome', 'Tokyo'];
    const apiKey = 'bab281d79e5f1e9755a68d754cc313e7';
    ...
    cityList.forEach(city => {
        ...
        cityItem.innerHTML = `
            ...
            <div class="city-weather">
                <span class="city-temp">Lädt...</span>
                <img class="city-icon" src="" alt="" style="display: none; width: 30px; height: 30px;">
            </div>
        `;

        // Hole Wetterdaten für die Stadt
        fetch(`https://api.openweathermap.org/data/2.5/weather?q=${city}&appid=${apiKey}&units=metric&lang=de`)
            .then(response => response.json())
            .then(data => {
                const temp = Math.round(data.main.temp); // Temperatur
                const iconCode = data.weather[0].icon;  // Icon
                const iconURL = `https://openweathermap.org/img/wn/${iconCode}.png`;

                // Aktualisiere die Temperatur und das Icon
                cityItem.querySelector('.city-temp').textContent = `${temp}°C`;
                const iconElement = cityItem.querySelector('.city-icon');
                iconElement.src = iconURL;
                iconElement.style.display = 'inline'; // Zeige das Icon an
            })
            .catch(error => {
                console.error(`Fehler beim Laden der Wetterdaten für ${city}:`, error);
                cityItem.querySelector('.city-temp').textContent = 'Daten fehlen';
            });

        // Füge Klickfunktion hinzu
        cityItem.addEventListener('click', () => {
            document.getElementById('city').value = city; // Setze Stadt ins Suchfeld
            fetchWeather(); // Rufe die Funktion für Wetterdaten auf
        });

        // Füge das Stadt-Element in die Liste ein
        popularcities.appendChild(cityItem);
    });
}

// Rufe `popcity` beim Laden der Seite auf
document.addEventListener('DOMContentLoaded', popcity);